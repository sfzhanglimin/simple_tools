...
const texture_2d_1 = __importDefault(require("./texture-2d"));
const texture_cube_1 = __importDefault(require("./texture-cube"));
const GL_INT = 5124;
const GL_FLOAT = 5126;
...
function _commitBlendStates(gl, cur, next) {
    ...
    if (cur.blend !== next.blend) {
        ...
        gl.enable(gl.BLEND);
        if (next.blendSrc === enums_1.enums.BLEND_CONSTANT_COLOR ||
            next.blendSrc === enums_1.enums.BLEND_ONE_MINUS_CONSTANT_COLOR ||
            next.blendDst === enums_1.enums.BLEND_CONSTANT_COLOR ||
            next.blendDst === enums_1.enums.BLEND_ONE_MINUS_CONSTANT_COLOR) {
...
function _commitDepthStates(gl, cur, next) {
    ...
    if (next.depthTest === false) {
        if (next.depthWrite) {
            next.depthTest = true;
            next.depthFunc = enums_1.enums.DS_FUNC_ALWAYS;
            gl.enable(gl.DEPTH_TEST);
            gl.depthFunc(next.depthFunc);
        }
        return;
...
function _commitStencilStates(gl, cur, next) {
    ...
    if (next.stencilTest === enums_1.enums.STENCIL_INHERIT) {
        return;
    }
    if (next.stencilTest !== cur.stencilTest) {
        if (next.stencilTest === enums_1.enums.STENCIL_DISABLE) {
...
function _commitCullMode(gl, cur, next) {
    ...
    if (next.cullMode === enums_1.enums.CULL_NONE) {
...
function _commitVertexBuffers(device, gl, cur, next) {
    ...
    if (cur.maxStream !== next.maxStream) {
        ...
        for (let i = 0; i < next.maxStream + 1; ++i) {
            if (cur.vertexBuffers[i] !== next.vertexBuffers[i] ||
...
function _attach(gl, location, attachment, face = 0) {
    if (attachment instanceof texture_2d_1.default) {
        gl.framebufferTexture2D(gl.FRAMEBUFFER, location, gl.TEXTURE_2D, attachment._glID, 0);
    }
    else if (attachment instanceof texture_cube_1.default) {
...
class Device {
    ...
    constructor(canvasEL, opts) {
        ...
        this._initStates();
        // runtime
        state_1.default.initDefault(this);
        this._current = new state_1.default(this);
        this._next = new state_1.default(this);
    ...
    clear(opts) {
        ...
        if (opts.depth !== undefined) {
            if (this._current.depthTest === false) {
                ...
                if (this._current.depthFunc !== enums_1.enums.DS_FUNC_ALWAYS) {
