...
const geom_utils_1 = require("../../core/geom-utils");
const enums_1 = __importDefault(require("../enums"));
let _tmp_mat4 = new value_types_1.Mat4();
let _matView = new value_types_1.Mat4();
let _matViewInv = new value_types_1.Mat4();
let _matProj = new value_types_1.Mat4();
let _matViewProj = new value_types_1.Mat4();
let _matInvViewProj = new value_types_1.Mat4();
let _tmp_v3 = new value_types_1.Vec3();
let _tmp2_v3 = new value_types_1.Vec3();
/**
...
class Camera {
    constructor() {
        this._poolID = -1;
        this._node = null;
        this._projection = enums_1.default.PROJ_PERSPECTIVE;
        // priority. the smaller one will be rendered first
        this._priority = 0;
        // clear options
        this._color = new value_types_1.Vec4(0.2, 0.3, 0.47, 1);
        this._depth = 1;
        this._stencil = 0;
     ...
     */
    getColor(out) {
        return value_types_1.Vec4.copy(out, this._color);
    }
    /**
     ...
     */
    setColor(r, g, b, a) {
        value_types_1.Vec4.set(this._color, r, g, b, a);
    }
    /**
    ...
    _calcMatrices(width, height) {
        ...
        value_types_1.Mat4.invert(_matView, _matViewInv);
        // projection matrix
        let aspect = width / height;
        if (this._projection === enums_1.default.PROJ_PERSPECTIVE) {
            value_types_1.Mat4.perspective(_matProj, this._fov, aspect, this._near, this._far);
        }
        else {
            let x = this._orthoHeight * aspect;
            let y = this._orthoHeight;
            value_types_1.Mat4.ortho(_matProj, -x, x, -y, y, this._near, this._far);
        }
        // view-projection
        value_types_1.Mat4.mul(_matViewProj, _matProj, _matView);
        // inv view-projection
        value_types_1.Mat4.invert(_matInvViewProj, _matViewProj);
    }
    /**
    ...
    extractView(out, width, height) {
        ...
        value_types_1.Mat4.copy(out._matView, _matView);
        value_types_1.Mat4.copy(out._matViewInv, _matViewInv);
        value_types_1.Mat4.copy(out._matProj, _matProj);
        value_types_1.Mat4.copy(out._matViewProj, _matViewProj);
        value_types_1.Mat4.copy(out._matInvViewProj, _matInvViewProj);
    ...
    screenPointToRay(x, y, width, height, out) {
        ...
        value_types_1.Vec3.set(_tmp2_v3, (x - cx) / cw * 2 - 1, (y - cy) / ch * 2 - 1, 1);
        value_types_1.Vec3.transformMat4(_tmp2_v3, _tmp2_v3, _matInvViewProj);
        if (this._projection === enums_1.default.PROJ_PERSPECTIVE) {
            // camera origin
            ...
            value_types_1.Vec3.set(_tmp_v3, (x - cx) / cw * 2 - 1, (y - cy) / ch * 2 - 1, -1);
            value_types_1.Vec3.transformMat4(_tmp_v3, _tmp_v3, _matInvViewProj);
        }
        return geom_utils_1.Ray.fromPoints(out, _tmp_v3, _tmp2_v3);
    }
    /**
    ...
    screenToWorld(out, screenPos, width, height) {
        ...
        if (this._projection === enums_1.default.PROJ_PERSPECTIVE) {
            ...
            value_types_1.Vec3.set(out, (screenPos.x - cx) / cw * 2 - 1, (screenPos.y - cy) / ch * 2 - 1, 0.9999);
            // transform to world
            value_types_1.Vec3.transformMat4(out, out, _matInvViewProj);
            // lerp to depth z
            this._node.getWorldPosition(_tmp_v3);
            value_types_1.Vec3.lerp(out, _tmp_v3, out, (0, value_types_1.lerp)(this._near / this._far, 1, screenPos.z));
        }
        else {
            value_types_1.Vec3.set(out, (screenPos.x - cx) / cw * 2 - 1, (screenPos.y - cy) / ch * 2 - 1, screenPos.z * 2 - 1);
            // transform to world
            value_types_1.Vec3.transformMat4(out, out, _matInvViewProj);
        }
        return out;
    ...
    worldToScreen(out, worldPos, width, height) {
        ...
        let cw = this._rect.w * width;
        let ch = this._rect.h * height;
        value_types_1.Vec3.transformMat4(out, worldPos, _matViewProj);
    ...
    worldMatrixToScreen(out, worldMatrix, width, height) {
        this._calcMatrices(width, height);
        value_types_1.Mat4.mul(out, _matViewProj, worldMatrix);
        let halfWidth = width / 2;
        let halfHeight = height / 2;
        value_types_1.Mat4.identity(_tmp_mat4);
        value_types_1.Mat4.transform(_tmp_mat4, _tmp_mat4, value_types_1.Vec3.set(_tmp_v3, halfWidth, halfHeight, 0));
        value_types_1.Mat4.scale(_tmp_mat4, _tmp_mat4, value_types_1.Vec3.set(_tmp_v3, halfWidth, halfHeight, 1));
        value_types_1.Mat4.mul(out, _tmp_mat4, out);
        return out;
    }
}
