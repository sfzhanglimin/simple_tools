...
const base_renderer_1 = __importDefault(require("../core/base-renderer"));
...
let _camPos = new value_types_1.Vec4(0, 0, 0, 0);
let _camFwd = new value_types_1.Vec3(0, 0, 0);
let _v3_tmp1 = new value_types_1.Vec3(0, 0, 0);
...
class ForwardRenderer extends base_renderer_1.default {
    ...
    _updateLights(scene) {
        ...
        for (let i = 0; i < lights.length; ++i) {
            ...
            if (light.shadowType !== enums_1.default.SHADOW_NONE) {
    ...
    _submitLightsUniforms() {
        ...
        if (this._lights.length > 0) {
            ...
            for (let i = 0; i < lightNum; ++i) {
                ...
                if (light._type === enums_1.default.LIGHT_SPOT) {
    ...
    _submitShadowStageUniforms(view) {
        ...
        this._device.setUniform('cc_shadow_map_lightViewProjMatrix', value_types_1.Mat4.toArray(_a16_viewProj, view._matViewProj));
    ...
    _submitOtherStagesUniforms() {
        ...
        for (let i = 0; i < this._shadowLights.length; ++i) {
            ...
            value_types_1.Mat4.toArray(view, light.viewProjMatrix);
            let index = i * 4;
    ...
    _opaqueStage(view, items) {
        ...
        // update uniforms
        this._device.setUniform('cc_matView', value_types_1.Mat4.toArray(_a16_view, view._matView));
        this._device.setUniform('cc_matViewInv', value_types_1.Mat4.toArray(_a16_view_inv, view._matViewInv));
        this._device.setUniform('cc_matProj', value_types_1.Mat4.toArray(_a16_proj, view._matProj));
        this._device.setUniform('cc_matViewProj', value_types_1.Mat4.toArray(_a16_viewProj, view._matViewProj));
        this._device.setUniform('cc_cameraPos', value_types_1.Vec4.toArray(_a4_camPos, _camPos));
        // update rendering
    ...
    _transparentStage(view, items) {
        ...
        for (let i = 0; i < items.length; ++i) {
            ...
            value_types_1.Vec3.sub(_v3_tmp1, _v3_tmp1, _camPos);
            item.sortKey = -value_types_1.Vec3.dot(_v3_tmp1, _camFwd);
        }
        this._sortItems(items);
        this._drawItems(view, items);
    }
