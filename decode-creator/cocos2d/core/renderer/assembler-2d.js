...
const manager_1 = __importDefault(require("./utils/dynamic-atlas/manager"));
const render_data_1 = __importDefault(require("./webgl/render-data"));
class Assembler2D extends assembler_1.default {
    constructor() {
        super();
    ...
    updateWorldVerts(comp) {
        ...
        if (justTranslate) {
            ...
            let al = a * vl, ar = a * vr, bl = b * vl, br = b * vr, cb = c * vb, ct = c * vt, db = d * vb, dt = d * vt;
            // left bottom
    ...
    packToDynamicAtlas(comp, frame) {
        if (CC_TEST)
            return;
        if (!frame._original && manager_1.default && frame._texture.packable && frame._texture.loaded) {
            let packedFrame = manager_1.default.insertSpriteFrame(frame);
            if (packedFrame) {
