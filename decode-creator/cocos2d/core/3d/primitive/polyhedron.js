 ...
 * @namespace primitive
 */
exports.PolyhedronType = cc.Enum({
    /**
...
const polyhedron = function (type, size = 1, opts = { sizeX: 0, sizeY: 0, sizeZ: 0 }) {
    ...
    let normals = [];
    let uvs = [];
    let minPos = new value_types_1.Vec3(Infinity, Infinity, Infinity);
    let maxPos = new value_types_1.Vec3(-Infinity, -Infinity, -Infinity);
    ...
    (0, utils_1.calcNormals)(positions, indices, normals);
    let boundingRadius = Math.sqrt(Math.pow(maxPos.x - minPos.x, 2), Math.pow(maxPos.y - minPos.y, 2), Math.pow(maxPos.z - minPos.z, 2));
