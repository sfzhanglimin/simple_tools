...
class OptimizedCurve {
    ...
    buildCurve(animationCurve, multiplier = 1) {
        ...
        if (this.optimizedKeys.length < keyNum) {
            ...
            for (i = 0; i < keyToAdd; i++) {
                const optKey = new curve_1.OptimizedKey();
        ...
        if (animationCurve.keyFrames.length === 1) {
            ...
            if (animationCurve.keyFrames[0].time !== 0) {
                this.optimizedKeys.unshift(new curve_1.OptimizedKey());
                this.optimizedKeys[0].time = 0;
            ...
            if (animationCurve.keyFrames[animationCurve.keyFrames.length - 1].time !== 1) {
                this.optimizedKeys.push(new curve_1.OptimizedKey());
    ...
    integrateOnce() {
        ...
        for (i = 0; i < this.integral.length; i++) {
            ...
            this.integral[i] = prevIntegral + (deltaT * (0, curve_1.evalOptCurve)(deltaT, this.optimizedKeys[i].coefficient));
        }
    ...
    evaluateIntegral(t, ts = 1) {
        t = (0, value_types_1.repeat)(t, 1);
        for (let i = 1; i < this.optimizedKeys.length; i++) {
            if (t < this.optimizedKeys[i].time) {
                const prevInt = i === 1 ? 0 : this.integral[i - 2];
                const dt = t - this.optimizedKeys[i - 1].time;
                return ts * (prevInt + (dt * (0, curve_1.evalOptCurve)(dt, this.optimizedKeys[i - 1].coefficient)));
            }
        }
        const dt = t - this.optimizedKeys[this.optimizedKeys.length - 1].time;
        return ts * (this.integral[this.integral.length - 1] + (dt * (0, curve_1.evalOptCurve)(dt, this.optimizedKeys[this.optimizedKeys.length - 1].coefficient)));
    }
    ...
    integrateTwice() {
        ...
        for (i = 0; i < this.integral.length; i++) {
            ...
            const deltaT = this.optimizedKeys[i + 1].time - this.optimizedKeys[i].time;
            const prevIntegral = i === 0 ? 0 : this.integral[i - 1];
            this.integral[i] = prevIntegral + (deltaT * deltaT * (0, curve_1.evalOptCurve)(deltaT, this.optimizedKeys[i].coefficient));
        }
    ...
    evaluateIntegralTwice(t, ts = 1) {
        ...
        for (let i = 1; i < this.optimizedKeys.length; i++) {
            if (t < this.optimizedKeys[i].time) {
                const prevInt = i === 1 ? 0 : this.integral[i - 2];
                const dt = t - this.optimizedKeys[i - 1].time;
                return ts * ts * (prevInt + (dt * dt * (0, curve_1.evalOptCurve)(dt, this.optimizedKeys[i - 1].coefficient)));
            }
        }
        const dt = t - this.optimizedKeys[this.optimizedKeys.length - 1].time;
        return ts * ts * (this.integral[this.integral.length - 1] + (dt * dt * (0, curve_1.evalOptCurve)(dt, this.optimizedKeys[this.optimizedKeys.length - 1].coefficient)));
    }
    updateKeyUniform() {
