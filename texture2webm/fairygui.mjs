import { gfx, Event as Event$1, Vec2, Node, director, macro, Color, Layers, Font, resources, Vec3, Rect, UITransform, UIOpacity, Component, Graphics, misc, Sprite, Size, screen, view, ImageAsset, AudioClip, AssetManager, assert, assetManager, BufferAsset, Asset, Texture2D, SpriteFrame, BitmapFont, sp, dragonBones, path, Label, LabelOutline, LabelShadow, SpriteAtlas, RichText, sys, EventMouse, game, EventTarget, Mask, math, isValid, View, EditBox, ParticleSystem2D } from 'cc';
...
class GTweener {
    ...
    update() {
        ...
        if (this._target && this._propType) {
            ...
            else if (typeof (this._propType) === "number") {
                if (this._target instanceof GObject) {
                    switch (this._propType) {
                        case 1:
                            {
                                this._target.x = this._value.x;
                            }
                            break;
                        case 2:
                            {
                                this._target.y = this._value.x;
                            }
                            break;
                        case 3:
                            {
                                this._target.setPosition(this._value.x, this._value.y);
                            }
                            break;
                        case 4:
                            {
                                this._target.width = this._value.x;
                            }
                            break;
                        case 5:
                            {
                                this._target.height = this._value.x;
                            }
                            break;
                        case 6:
                            {
                                this._target.setSize(this._value.x, this._value.y);
                            }
                            break;
                        case 7:
                            {
                                this._target.setScale(this._value.x, this._target.scale.y);
                            }
                            break;
                        case 8:
                            {
                                this._target.setScale(this._target.scale.x, this._value.x);
                            }
                            break;
                        case 9:
                            this._target.setScale(this._value.x, this._value.y);
                            break;
                        case 10:
                            {
                                this._target.rotation = this._value.x;
                            }
                            break;
                        case 11:
                            {
                                this._target.alpha = this._value.x;
                            }
                            break;
                        case 12:
                            {
                                if (typeof ((this._target).update) === "function") {
                                    (this._target).update(this._value.x);
                                }
                            }
                            break;
                    }
                }
                else if (this._target instanceof Node) {
                    switch (this._propType) {
                        case 1:
                            {
                                this._target.setPosition(this._value.x, this._target.position.y, this._target.position.z);
                            }
                            break;
                        case 2:
                            {
                                this._target.setPosition(this._target.position.x, this._value.x, this._target.position.z);
                            }
                            break;
                        case 3:
                            {
                                this._target.setPosition(this._value.x, this._value.y, this._target.position.z);
                            }
                            break;
                        case 7:
                            {
                        ...
                        case 10:
                            {
                                this._target.angle = this._value.x;
                            }
                            break;
                    }
                }
...
class GTween {
    ...
    static To(start, end, duration) {
        if (typeof (start) === "object") {
            ...
            let type1 = null;
            let type2 = null;
            for (let i = 0; i < ALL_FILED.length; ++i) {
                type1 = typeof (start[ALL_FILED[i]]) === "number";
                type2 = typeof (end[ALL_FILED[i]]) === "number";
                if (type1 && type2) {
                    arg1.push(start[ALL_FILED[i]]);
                    arg2.push(end[ALL_FILED[i]]);
                }
                else if (type1 || type2) {
                    console.warn(`传入参数没有对齐,忽略key值:${ALL_FILED[i]}`);
                }
            }
            let count = arg1.length;
            let funcName = "_to" + (count > 1 ? count : "");
            const t = TweenManager.createTween();
            arg2.push(duration);
            return t[funcName].call(t, ...(arg1.concat(arg2)));
        }
        else {
            return TweenManager.createTween()._to(start, end, duration);
        }
    }
    static ToDouble(start, end, duration) {
        return this.to(start, end, duration);
    }
    ...
    static shake(...args) {
        if (typeof (args[0]) === "number") {
            return TweenManager.createTween()._shake(args[0], args[1], args[2], args[3]);
        }
        else {
            return TweenManager.createTween()._shake(args[0].x, args[0].y, args[1], args[2]);
        }
    }
    static isTweening(target, propType) {
...
class RelationItem {
    ...
    copyFrom(source) {
        ...
        this._defs.length = 0;
        let length = source._defs.length;
        for (let i = 0; i < length; i++) {
            let info = source._defs[i];
    ...
    applyOnSelfResized(dWidth, dHeight, applyPivot) {
        let ox = this._owner.x;
        let oy = this._owner.y;
        let length = this._defs.length;
        for (let i = 0; i < length; i++) {
            let info = this._defs[i];
            switch (info.type) {
        ...
        if (ox != this._owner.x || oy != this._owner.y) {
            ...
            if (this._owner.parent) {
                let len = this._owner.parent._transitions.length;
                if (len > 0) {
                    for (let i = 0; i < len; ++i) {
    ...
    __targetXYChanged(evt) {
        ...
        let ox = this._owner.x;
        let oy = this._owner.y;
        let dx = this._target.x - this._targetX;
        let dy = this._target.y - this._targetY;
        let length = this._defs.length;
        for (let i = 0; i < length; i++) {
            let info = this._defs[i];
    ...
    __targetSizeChanged(evt) {
        ...
        let ox = this._owner.x;
        let oy = this._owner.y;
        let ow = this._owner._rawWidth;
        let oh = this._owner._rawHeight;
        let length = this._defs.length;
        for (let i = 0; i < length; i++) {
            let info = this._defs[i];
...
class Relations {
    ...
    add(target, relationType, usePercent) {
        ...
        let newItem = new RelationItem(this._owner);
    ...
    copyFrom(source) {
        this.clearAll();
        let arr = source._items;
        let length = arr.length;
        for (let i = 0; i < length; i++) {
            let ri = arr[i];
            let item = new RelationItem(this._owner);
            item.copyFrom(ri);
...
UIConfig.onMusicCallback = () => { return true; };
let _fontRegistry = {};
...
class GObject {
    ...
    //add by zlm
    get xy() {
        return { x: this._x, y: this._y, z: 1 };
    }
    set xy(value) {
    ...
    setSize(wv, hv, ignorePivot) {
        if (this._rawWidth != wv || this._rawHeight != hv) {
            ...
            if (this.maxHeight > 0 && hv > this.maxHeight)
                hv = this.maxHeight;
            let dWidth = wv - this._width;
            let dHeight = hv - this._height;
            this._width = wv;
            this._height = hv;
    ...
    //add by zlm
    set pivotAsAnchor(value) {
        this.setPivot(this.pivotX, this.pivotY, value);
    }
    ...
    addDisplayLock() {
        let gearDisplay = this._gears[0];
        if (gearDisplay && gearDisplay.controller) {
            let ret = gearDisplay.addLock();
    ...
    checkGearDisplay() {
        if (this._handlingController)
            return;
        let connected = this._gears[0] == null || this._gears[0].connected;
        if (this._gears[8])
    ...
    LocalToRoot(aPos) {
        var _a, _b;
        return this.localToGlobal((_a = aPos === null || aPos === void 0 ? void 0 : aPos.x) !== null && _a !== void 0 ? _a : 0, (_b = aPos === null || aPos === void 0 ? void 0 : aPos.y) !== null && _b !== void 0 ? _b : 0);
    }
    ...
    RootToLocal(aPos) {
        var _a, _b;
        return this.globalToLocal((_a = aPos === null || aPos === void 0 ? void 0 : aPos.x) !== null && _a !== void 0 ? _a : 0, (_b = aPos === null || aPos === void 0 ? void 0 : aPos.y) !== null && _b !== void 0 ? _b : 0);
    }
    ...
    setup_afterAdd(buffer, beginPos) {
        ...
        for (let i = 0; i < cnt; i++) {
            ...
            let gear = this.getGear(buffer.readByte());
            gear.setup(buffer);
    ...
    onTouchMove_0(evt) {
        ...
        if (GObject.draggingObject == this) {
            ...
            if (this._dragBounds) {
                let rect = Decls$1.GRoot.inst.localToGlobalRect(this._dragBounds.x, this._dragBounds.y, this._dragBounds.width, this._dragBounds.height, s_rect$1);
                if (xx < rect.x)
                    xx = rect.x;
...
let s_vec2$4 = new Vec2();
let s_vec3$1 = new Vec3();
s_vec3$1.z = 0;
let s_rect$1 = new Rect();
let sGlobalDragStart = new Vec2();
let sGlobalRect = new Rect();
let s_dragging;
let s_dragQuery;
let Decls$1 = {};
let constructingDepth = { n: 0 };
...
class GGroup extends GObject {
    ...
    updateBounds() {
        ...
        let cnt = this._parent.numChildren;
        let i;
        let child;
    ...
    handleAlphaChanged() {
        ...
        let cnt = this._parent.numChildren;
        for (let i = 0; i < cnt; i++) {
            let child = this._parent.getChildAt(i);
            if (child.group == this)
...
class GGraph extends GObject {
    ...
    drawRect(lineSize, lineColor, fillColor, corner) {
        ...
        //add by zlm
        this.setLColor(lineColor);
        this.setFColor(fillColor);
    ...
    set color(value) {
        let color = new Color(value.r, value.g, value.b, value.a * this.alpha);
     ...
     */
    setLColor(value) {
        let color = new Color(value.a, value.g, value.b, value.a * this.alpha);
    ...
    updateGraph() {
        ...
        if (w == 0 || h == 0)
            return;
        let px = -this.pivotX * this._width;
        let py = this.pivotY * this._height;
        ...
        if (this._type == 1) {
            ...
            if (!this._polygonPoints)
                this._polygonPoints = [];
            let radius = Math.min(w, h) / 2 - ls;
            this._polygonPoints.length = 0;
            let angle = misc.degreesToRadians(this._startAngle);
            let deltaAngle = 2 * Math.PI / this._sides;
            let dist;
            for (let i = 0; i < this._sides; i++) {
                if (this._distances) {
                    ...
                    dist = 1;
                let xv = radius + radius * dist * Math.cos(angle);
                let yv = radius + radius * dist * Math.sin(angle);
    ...
    setup_beforeAdd(buffer, beginPos) {
        ...
        if (this._type != 0) {
            ...
            //add by zlm
            this.setLColor(buffer.readColor(true));
            this.setFColor(buffer.readColor(true));
...
class GImage extends GObject {
    ...
    constructFromResource() {
        let contentItem = this.packageItem.getBranch();
    ...
    setupWebm() {
        ...
        if (((_a = this.data) === null || _a === void 0 ? void 0 : _a.length) > 0) {
            const list = this.data.match(WEBM_FIND_NAME_REG);
            if (list && list.length > 1) {
                this.node.addComponent("WebmComponent");
            }
        }
    }
}
const WEBM_FIND_NAME_REG = /webm\s*[:=]\s*([a-zA-Z0-9_-]+)/;
...
function updateScaler() {
    ...
    let ss = Math.max(view.getScaleX(), view.getScaleY());
...
class PackageItem {
    ...
    getBranch() {
        if (this.branches && this.owner._branchIndex != -1) {
            let itemId = this.branches[this.owner._branchIndex];
            if (itemId)
    ...
    getHighResolution() {
        if (this.highResolution && UIContentScaler.scaleLevel > 0) {
            let itemId = this.highResolution[UIContentScaler.scaleLevel - 1];
            if (itemId)
                return this.owner.getItemById(itemId);
        }
        return this;
...
class TranslationHelper {
    static loadFromXML(source) {
        ...
        let xml = new DOMParser().parseFromString(source, "text/xml").documentElement;
        let nodes = xml.childNodes;
        let length1 = nodes.length;
        for (let i1 = 0; i1 < length1; i1++) {
            let cxml = nodes[i1];
            if (cxml.tagName == "string") {
                let key = cxml.getAttribute("name");
                let text = cxml.childNodes.length > 0 ? cxml.firstChild.nodeValue : "";
                let i = key.indexOf("-");
                if (i == -1)
                    continue;
    ...
    static translateComponent(item) {
        ...
        let page;
        buffer.seek(0, 2);
        let childCount = buffer.readShort();
                ...
                case ObjectType.Tree:
                    {
                        ...
                        for (j = 0; j < itemCount; j++) {
                            ...
                            if (buffer.version >= 2) {
                                ...
                                let cpCount = buffer.readUshort();
                                for (let k = 0; k < cpCount; k++) {
                                    let target = buffer.readS();
                                    let propertyId = buffer.readUshort();
...
class UIPackage {
     ...
     */
    static addPackage(...aArgs) {
        let bundle = null;
        let path = null;
        if (aArgs.length > 1) {
            if (aArgs[0] instanceof AssetManager.Bundle) {
                bundle = aArgs[0];
                path = aArgs[1];
            }
            else {
                assert(typeof (aArgs[0]) === "string", "传入的第一个参数必须是bundle后者string");
                bundle = assetManager.getBundle(aArgs[0]);
                path = aArgs[1];
            }
        }
        else {
        ...
        if (pkg)
            return pkg;
        let asset = bundle.get(PathUtils.join("im", path), BufferAsset);
        if (!asset)
    ...
    /**add by zlm */
    static AddPackage(sepStr) {
        const list = sepStr.split("/");
        return this.addPackage(list[0], list[1]);
    }
    ...
    static removePackage(packageIdOrName) {
        let pkg = _instById[packageIdOrName];
        if (!pkg)
    ...
    static removeAllPackages() {
        Object.keys(_instByName).forEach(aKey => {
            _instByName[aKey].dispose();
        });
        _instById = {};
        _instByName = {};
    }
    ...
    /**add by zlm */
    static CreateObject(pkgName, resName, userClass) {
        return this.createObject(pkgName, resName, userClass);
    }
    ...
    static getItemURL(pkgName, resName) {
        ...
        if (!pkg)
            return null;
        let pi = pkg._itemsByName[resName];
        if (!pi)
            return null;
    ...
    loadPackage(buffer, path) {
        ...
        for (i = 0; i < cnt; i++) {
            ...
            if (pi.file) {
                let info = this._bundle.getInfoWithPath(pi.file);
                if (!info) {
                    let temp = PathUtils.join("im", pi.file);
                    info = this._bundle.getInfoWithPath(temp);
                    if (!info) {
                        temp = PathUtils.join("delay", pi.file);
                    }
                    pi.file = temp;
                }
            }
    ...
    createObject(resName, userClass) {
        let pi = this._itemsByName[resName];
        if (pi)
    ...
    internalCreateObject(item, userClass) {
        let g = Decls.UIObjectFactory.newObject(item, userClass);
        if (g == null)
            return null;
        constructingDepth.n++;
            ...
            case PackageItemType.Sound:
                if (!item.decoded) {
                    ...
                    if (!item.asset)
                        ...
                        const texture = this._bundle.get(item.file + "/texture", Texture2D);
    ...
    loadFont(item) {
        ...
        buffer.seek(0, 1);
        let bg;
        let cnt = buffer.readInt();
        for (let i = 0; i < cnt; i++) {
            let nextPos = buffer.readShort();
            nextPos += buffer.position;
            bg = {};
            let ch = buffer.readUshort();
            dict[ch] = bg;
            let rect = new Rect();
            bg.rect = rect;
    ...
    getSpine(item) {
        const asset = this._bundle.get(item.file, sp.SkeletonData);
        item.decoded = true;
        item.asset = asset;
        return asset;
    }
    getDragonBones(item) {
        const asset = this._bundle.get(item.file, dragonBones.DragonBonesAsset);
        item.decoded = true;
        item.asset = asset;
        return asset;
    }
    ...
    set font(value) {
        if (this._font != value || !value) {
            ...
            if (newFont.startsWith("ui://")) {
                let pi = UIPackage.getItemByURL(newFont);
                if (pi)
            ...
            this.updateFont();
            /**add by zlm fixed default font size are not resize */
            this.updateFontSize();
        }
    }
    get fontSize() {
    ...
    enter(controller) {
        ...
        if (trans) {
            if (this._currentTransition && this._currentTransition.playing)
                ...
                trans.play(this.playTimes, this.delay);
...
class Controller extends EventTarget {
    ...
    set selectedPage(val) {
        let i = this._pageNames.indexOf(val);
        if (i == -1)
            i = 0;
    ...
    removePage(name) {
        let i = this._pageNames.indexOf(name);
        if (i != -1) {
    ...
    getPageIdByName(aName) {
        let i = this._pageNames.indexOf(aName);
        if (i != -1)
    ...
    getPageNameById(aId) {
        let i = this._pageIds.indexOf(aId);
        if (i != -1)
    ...
    set selectedPageId(val) {
        let i = this._pageIds.indexOf(val);
        this.selectedIndex = i;
    }
    ...
    setup(buffer) {
        ...
        if (cnt > 0) {
            ...
            for (i = 0; i < cnt; i++) {
                nextPos = buffer.readShort();
                nextPos += buffer.position;
                let action = createAction(buffer.readByte());
                action.setup(buffer);
...
class ScrollPane extends Component {
    ...
    setup(buffer) {
        ...
        let vtScrollBarRes = buffer.readS();
        let hzScrollBarRes = buffer.readS();
        let headerRes = buffer.readS();
        let footerRes = buffer.readS();
        if ((flags & 1) != 0)
        ...
        if (scrollBarDisplay != ScrollBarDisplayType.Hidden) {
            if (this._scrollType == ScrollType.Both || this._scrollType == ScrollType.Vertical) {
                let res = vtScrollBarRes ? vtScrollBarRes : UIConfig.verticalScrollBar;
                if (res) {
            ...
            if (this._scrollType == ScrollType.Both || this._scrollType == ScrollType.Horizontal) {
                let res = hzScrollBarRes ? hzScrollBarRes : UIConfig.horizontalScrollBar;
                if (res) {
    ...
    isChildInView(obj) {
        if (this._overlapSize.y > 0) {
            let dist = obj.y + (-this._container.position.y);
            if (dist < -obj.height || dist > this._viewSize.y)
                return false;
        }
        if (this._overlapSize.x > 0) {
            let dist = obj.x + this._container.position.x;
    ...
    changeContentSizeOnScrolling(deltaWidth, deltaHeight, deltaPosX, deltaPosY) {
        let isRightmost = this._xPos == this._overlapSize.x;
        let isBottom = this._yPos == this._overlapSize.y;
    ...
    handleSizeChanged() {
        ...
        let maskWidth = this._viewSize.x;
        let maskHeight = this._viewSize.y;
    ...
    onTouchBegin(evt) {
        ...
        if (this._tweening != 0) {
            ...
            this._dragged = false;
        let pt = this._owner.globalToLocal(evt.pos.x, evt.pos.y, s_vec2$3);
    ...
    onTouchMove(evt) {
        ...
        //更新速度
        let now = game.totalTime / 1000;
        let deltaTime = Math.max(now - this._lastMoveTime, 1 / 60);
        let deltaPositionX = pt.x - this._lastTouchPos.x;
        let deltaPositionY = pt.y - this._lastTouchPos.y;
        if (!sh)
            deltaPositionX = 0;
        if (!sv)
            deltaPositionY = 0;
        if (deltaTime != 0) {
            let frameRate = 60;
            let elapsed = deltaTime * frameRate - 1;
            if (elapsed > 1) //速度衰减
             {
                let factor = Math.pow(0.833, elapsed);
        ...
        */
        let deltaGlobalPositionX = this._lastTouchGlobalPos.x - evt.pos.x;
        let deltaGlobalPositionY = this._lastTouchGlobalPos.y - evt.pos.y;
        if (deltaPositionX != 0)
    ...
    onTouchEnd(evt) {
        ...
        if (flag) {
            ...
            if (!this._inertiaDisabled) {
                let frameRate = 60;
                let elapsed = (game.totalTime / 1000 - this._lastMoveTime) * frameRate - 1;
                if (elapsed > 1) {
                    let factor = Math.pow(0.833, elapsed);
                    this._velocity.x = this._velocity.x * factor;
                    this._velocity.y = this._velocity.y * factor;
    ...
    alignPosition(pos, inertialScrolling) {
        ...
        if (this._pageMode) {
            ...
            let pt = this._owner.getSnappingPosition(-pos.x, -pos.y, s_vec2$3);
    ...
    alignByPage(pos, axis, inertialScrolling) {
        ...
        if (pos > 0)
            ...
            let change = inertialScrolling ? (pos - this._containerPos[axis]) : (pos - (axis == "x" ? this._container.position.x : (-this._container.position.y)));
    ...
    checkRefreshBar() {
        if (this._header == null && this._footer == null)
            return;
        let pos = (this._refreshBarAxis == "x" ? this._container.position.x : (-this._container.position.y));
        if (this._header) {
            if (pos > 0) {
    ...
    update(dt) {
        if (this._tweening == 0)
            return;
        let nx = this.runTween("x", dt);
        let ny = this.runTween("y", dt);
...
let s_rect = new Rect();
let sEndPos = new Vec2();
let sOldChange = new Vec2();
...
class Transition {
    ...
    play(...aArgs) {
        ...
        switch (typeof aArgs[0]) {
            case "number":
                {
                    if (aArgs.length <= 3) {
                        this._play(aArgs[2], aArgs[0], aArgs[1]);
                    }
                    else if (aArgs.length !== 4) {
                        this._play(aArgs[4], aArgs[0], aArgs[1], aArgs[2], aArgs[3]);
                    }
                    else {
                        console.warn("transtion play 参数不对.", aArgs);
                    }
                }
                break;
            case "function":
        ...
        if (this._autoPlay != value) {
            ...
            if (this._autoPlay) {
                if (this._owner.onStage)
                    this.play(this._autoPlayTimes, this._autoPlayDelay);
            }
            else {
                if (!this._owner.onStage)
    ...
    _play(onComplete, times, delay, startTime, endTime, reversed) {
        ...
        for (let i = 0; i < cnt; i++) {
            ...
            if (item.target && item.type == ActionType.Transition) {
                let trans = item.target.getTransition(item.value.transName);
                if (trans == this)
                    trans = null;
                if (trans) {
    ...
    setPaused(paused) {
        if (!this._playing || this._paused == paused)
            return;
        this._paused = paused;
        let tweener = GTween.getTween(this);
        if (tweener)
        ...
        switch (item.type) {
            ...
            case ActionType.Color:
                ...
                if (color instanceof Color) {
                    ...
                    const g = (i >> 8) & 0xFF;
                    const b = i & 0xFF;
                    item.target.setProp(ObjectPropID.Color, new Color(r, g, b, color.a));
                }
                break;
            case ActionType.Animation:
                if (value.frame >= 0)
                ...
                if (value.animationName) {
                    item.target.animationName = value.animationName;
                }
                if (value.skinName) {
                    item.target.skinName = value.skinName;
                }
                break;
            case ActionType.Visible:
            ...
            case ActionType.Transition:
                if (this._playing) {
                    ...
                    if (trans) {
                        this._totalTasks++;
                        let startTime = this._startTime > item.time ? (this._startTime - item.time) : 0;
            ...
            case ActionType.Sound:
                if (this._playing && item.time >= this._startTime) {
                    let pi = UIPackage.getItemByURL(value.sound);
                    if (pi) {
                        Decls$1.GRoot.inst.playOneShotSound(pi.file, value.volume);
                    }
                }
                break;
            case ActionType.Shake:
    ...
    setup(buffer) {
        ...
        for (let i = 0; i < cnt; i++) {
            ...
            buffer.seek(curPos, 0);
            let item = new Item(buffer.readByte());
            this._items[i] = item;
...
class GComponent extends GObject {
    ...
    dispose() {
        ...
        for (i = 0; i < cnt; ++i) {
            let trans = this._transitions[i];
            trans.dispose();
        }
        cnt = this._controllers.length;
        for (i = 0; i < cnt; ++i) {
            let cc = this._controllers[i];
            cc.dispose();
        }
        if (this._scrollPane)
    ...
    getController(name) {
        let cnt = this._controllers.length;
        for (let i = 0; i < cnt; ++i) {
            let c = this._controllers[i];
            if (c.name == name)
                return c;
        }
        return null;
    ...
    removeController(c) {
        let index = this._controllers.indexOf(c);
        if (index == -1)
    ...
    get baseUserData() {
        let buffer = this.packageItem.rawData;
        buffer.seek(0, 4);
        return buffer.readS();
    }
    setupScroll(buffer) {
    ...
    /**add by zlm */
    setupOverflowHidden(value) {
        if (value) {
            this.setupOverflow(OverflowType.Hidden);
        }
    ...
    _hitTest(pt, globalPt) {
        ...
        if (this.hitArea) {
            ...
            const transCom = this._container._uiProps.uiTransformComp;
            const clippingSize = transCom.contentSize;
            const parentAnchor = this.node._uiProps.uiTransformComp.anchorPoint;
            s_vec2$2.set(pt);
            s_vec2$2.x += this._container.position.x - ((transCom.anchorX - parentAnchor.x) * clippingSize.width);
            s_vec2$2.y += this._container.position.y - ((transCom.anchorY - parentAnchor.y) * clippingSize.height);
    ...
    constructFromResource2(objectPool, poolIndex) {
        ...
        let i1;
        let i2;
        let buffer = contentItem.rawData;
        buffer.seek(0, 0);
        ...
        let overflow = buffer.readByte();
        if (overflow == OverflowType.Scroll) {
            let savedPos = buffer.position;
            buffer.seek(0, 7);
        ...
        buffer.seek(0, 1);
        let controllerCount = buffer.readShort();
        ...
        for (i = 0; i < childCount; i++) {
            ...
            if (objectPool)
                ...
                let src = buffer.readS();
                let pkgId = buffer.readS();
                let pi = null;
                if (src != null) {
                    let pkg;
                    if (pkgId != null)
        ...
        let maskId = buffer.readShort();
        if (maskId != -1) {
            this.setMask(this.getChildAt(maskId), buffer.readBool());
        }
        let hitTestId = buffer.readS();
        i1 = buffer.readInt();
        i2 = buffer.readInt();
        if (hitTestId != null) {
            let pi = contentItem.owner.getItemById(hitTestId);
        ...
        buffer.seek(0, 5);
        let transitionCount = buffer.readShort();
    ...
    setup_afterAdd(buffer, beginPos) {
        ...
        let pageController = buffer.readShort();
        if (pageController != -1 && this._scrollPane)
            this._scrollPane.pageController = this._parent.getControllerAt(pageController);
        let cnt = buffer.readShort();
        for (let i = 0; i < cnt; i++) {
            let cc = this.getController(buffer.readS());
            let pageId = buffer.readS();
            if (cc)
        ...
        if (buffer.version >= 2) {
            ...
            for (let i = 0; i < cnt; i++) {
                ...
                let obj = this.getChildByPath(target);
                if (obj)
...
class Window extends GComponent {
    ...
    hideImmediately() {
        let r = (this.parent instanceof GRoot) ? this.parent : null;
        if (!r)
            r = GRoot.inst;
    ...
    layoutModalWaitPane() {
        if (this._contentArea) {
            let pt = this._frame.localToGlobal();
    ...
    init() {
        ...
        if (this._uiSources.length > 0) {
            this._loading = false;
            let cnt = this._uiSources.length;
            for (let i = 0; i < cnt; i++) {
                let lib = this._uiSources[i];
                if (!lib.loaded) {
    ...
    closeEventHandler(evt) {
        if (UIConfig.onMusicCallback) {
            UIConfig.onMusicCallback("closeButton", 1);
        }
        this.hide();
    }
    onEnable() {
    ...
    onDragStart_1(evt) {
        let original = GObject.cast(evt.currentTarget);
        original.stopDrag();
...
class GRoot extends GComponent {
    ...
    playOneShotSound(clipPath, volumeScale) {
        // if (!this.audioEngine) {
        ...
        // }
        if (clipPath && UIConfig.onMusicCallback) {
            UIConfig.onMusicCallback(clipPath, volumeScale);
            // this.audioEngine.playOneShot(clip,this._volumeScale * volumeScale);
...
class GObjectPool {
    ...
    getObject(url) {
        ...
        let child = UIPackage.createObjectFromURL(url);
        return child;
    }
    returnObject(obj) {
...
class GLoader extends GObject {
    ...
    loadFromPackage(itemURL) {
        ...
        if (this._contentItem) {
            ...
            if (this._contentItem.type == PackageItemType.Image) {
                ...
                let obj = UIPackage.createObjectFromURL(itemURL);
                if (!obj)
                    this.setErrorState();
    ...
    clearContent() {
        ...
        if (!this._contentItem) {
            let texture = this._content.spriteFrame;
            if (texture)
...
class GLoader3D extends GObject {
    ...
    loadFromPackage(itemURL) {
        ...
        if (this._contentItem) {
            ...
            if (this._contentItem.type == PackageItemType.Spine || this._contentItem.type == PackageItemType.DragonBones) {
                ...
                this.onLoaded(null, this._contentItem);
            }
        }
    }
    onLoaded(err, item) {
    ...
    setParcitle(asset, anchor) {
        ...
        this._content = node.addComponent(ParticleSystem2D);
        this._content.file = asset;
        this._content.color = this._color;
        this.onChangeParcitle();
        this.updateLayout();
    }
    onChangeParcitle() {
        if (this._content instanceof ParticleSystem2D) {
            if (this.playing) {
                this._content.resetSystem();
            }
            else {
                this._content.stopSystem();
            }
        }
    }
    freeParcitle() {
    ...
    setScale(aX, aY) {
        if (!(this._content instanceof ParticleSystem2D)) {
            super.setScale(aX, aY);
            return;
        }
        if (this._parcitleOriScale !== aX) {
            this._content.startSize = this._content.startSize / this._parcitleOriScale * aX;
            this._content.startSizeVar = this._content.startSizeVar / this._parcitleOriScale * aX;
            let size = this._content.posVar;
            size.x = size.x / this._parcitleOriScale * aX;
            size.y = size.y / this._parcitleOriScale * aX;
            this._content.posVar = size;
            this._parcitleOriScale = aX;
            this.onChangeParcitle();
            this.updateLayout();
        }
    }
    setSpine(asset, anchor, pma) {
        ...
        this._content.premultipliedAlpha = false;
    ...
    onChangeSpine() {
        ...
        if (this._animationName) {
            ...
            if (this._playing)
                ...
                this._content.updateAnimation(0);
        ...
        const skinName = (_b = (_a = this._content._skeleton.skin) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : "default";
        if (skinName != skin)
...
class GLabel extends GComponent {
    ...
    onClick_1() {
        if (this._sound) {
            let pi = UIPackage.getItemByURL(this._sound);
            if (pi) {
...
class GButton extends GComponent {
    ...
    set selected(val) {
        ...
        if (this._selected != val) {
            ...
            if (this._selectedIcon) {
                let str = this._selected ? this._selectedIcon : this._icon;
                if (this._iconObject)
    ...
    getTextField() {
        if (this._titleObject instanceof GTextField)
            return this._titleObject;
        else if (this._titleObject && 'getTextField' in this._titleObject)
            return this._titleObject.getTextField();
        else
            return null;
    }
    ...
    setState(val) {
        ...
        if (this._downEffect == 1) {
            ...
            if (val == GButton.DOWN || val == GButton.SELECTED_OVER || val == GButton.SELECTED_DISABLED) {
                ...
                let r = this._downEffectValue * 255;
...
class GList extends GComponent {
    ...
    removeChildAt(index, dispose) {
        let child = super.removeChildAt(index, dispose);
        if (!dispose)
    ...
    onClickItem(evt) {
        if (this._scrollPane && this._scrollPane.isDragged)
            return;
        let item = GObject.cast(evt.currentTarget);
    ...
    setSelectionOnEvent(item, evt) {
        ...
        let index = this.childIndexToItemIndex(this.getChildIndex(item));
        if (this._selectionMode == ListSelectionMode.Single) {
            if (!item.selected) {
    ...
    resizeToFit(itemCount = Number.POSITIVE_INFINITY, minSize = 0) {
        ...
        if (itemCount > curCount)
            itemCount = curCount;
        if (this._virtual) {
            let lineCount = Math.ceil(itemCount / this._curLineItemCount);
    ...
    updateSelectionController(index) {
        if (this._selectionController && !this._selectionController.changing
            ...
            let c = this._selectionController;
    ...
    scrollToView(index, ani, setFirst) {
        if (this._virtual) {
            ...
            if (this._layout == ListLayoutType.SingleColumn || this._layout == ListLayoutType.FlowHorizontal) {
                ...
                let page = index / (this._curLineItemCount * this._curLineItemCount2);
    ...
    set numItems(value) {
        if (this._virtual) {
            ...
            let oldCount = this._virtualItems.length;
    ...
    _refreshVirtualList(dt) {
        ...
        let layoutChanged = this._virtualListChanged == 2;
        ...
        if (this._realNumItems > 0) {
            let i;
            let len = Math.ceil(this._realNumItems / this._curLineItemCount) * this._curLineItemCount;
            let len2 = Math.min(this._curLineItemCount, this._realNumItems);
            if (this._layout == ListLayoutType.SingleColumn || this._layout == ListLayoutType.FlowHorizontal) {
                ...
                let pageCount = Math.ceil(len / (this._curLineItemCount * this._curLineItemCount2));
    ...
    getIndexOnPos3(forceUpdate) {
        ...
        let page = Math.floor(s_n / viewWidth);
        let startIndex = page * (this._curLineItemCount * this._curLineItemCount2);
    ...
    handleScroll1(forceUpdate) {
        ...
        let end = max == this._scrollPane.contentHeight; //这个标志表示当前需要滚动到最末，无论内容变化大小
        //寻找当前位置的第一条项目
        s_n = pos;
        ...
        let ii, ii2;
        let i, j;
        let partSize = (this._scrollPane.viewWidth - this._columnGap * (this._curLineItemCount - 1)) / this._curLineItemCount;
        this.itemInfoVer++;
    ...
    handleScroll2(forceUpdate) {
        ...
        let end = pos == this._scrollPane.contentWidth; //这个标志表示当前需要滚动到最末，无论内容变化大小
        //寻找当前位置的第一条项目
        s_n = pos;
        ...
        let ii, ii2;
        let i, j;
        let partSize = (this._scrollPane.viewHeight - this._lineGap * (this._curLineItemCount - 1)) / this._curLineItemCount;
        this.itemInfoVer++;
    ...
    handleScroll3(forceUpdate) {
        ...
        let virtualItemCount = this._virtualItems.length;
        let pageSize = this._curLineItemCount * this._curLineItemCount2;
        let startCol = newFirstIndex % this._curLineItemCount;
        let viewWidth = this.viewWidth;
        let page = Math.floor(newFirstIndex / pageSize);
        let startIndex = page * pageSize;
        let lastIndex = startIndex + pageSize * 2; //测试两页
        let needRender;
        let i;
        let ii, ii2;
        let col;
        let url = this._defaultItem;
        let partWidth = (this._scrollPane.viewWidth - this._columnGap * (this._curLineItemCount - 1)) / this._curLineItemCount;
        let partHeight = (this._scrollPane.viewHeight - this._lineGap * (this._curLineItemCount2 - 1)) / this._curLineItemCount2;
        this.itemInfoVer++;
        //先标记这次要用到的项目
    ...
    handleArchOrder1() {
        if (this._childrenRenderOrder == ChildrenRenderOrder.Arch) {
            let mid = this._scrollPane.posY + this.viewHeight / 2;
    ...
    handleArchOrder2() {
        if (this._childrenRenderOrder == ChildrenRenderOrder.Arch) {
            let mid = this._scrollPane.posX + this.viewWidth / 2;
            let minDist = Number.POSITIVE_INFINITY;
            let dist = 0;
            let apexIndex = 0;
            let cnt = this.numChildren;
...
class GComboBox extends GComponent {
    ...
    setup_afterAdd(buffer, beginPos) {
        ...
        let nextPos;
        let str;
        let itemCount = buffer.readShort();
...
class GSlider extends GComponent {
    ...
    updateWithPercent(percent, manual) {
        ...
        if (manual) {
            let newValue = math.clamp(this._min + (this._max - this._min) * percent, this._min, this._max);
    ...
    onGripTouchMove(evt) {
        ...
        let pt = this.globalToLocal(evt.pos.x, evt.pos.y, s_vec2$1);
    ...
    onBarTouchBegin(evt) {
        if (!this.changeOnClick)
            return;
        let pt = this._gripObject.globalToLocal(evt.pos.x, evt.pos.y, s_vec2$1);
        let percent = math.clamp01((this._value - this._min) / (this._max - this._min));
        let delta = 0;
...
class GProgressBar extends GComponent {
    ...
    tweenValue(value, duration) {
        let oldValule;
        let tweener = GTween.getTween(this, this.update);
        if (tweener) {
    ...
    update(newValue) {
        let percent = math.clamp01((newValue - this._min) / (this._max - this._min));
        if (this._titleObject) {
            switch (this._titleType) {
                case ProgressTitleType.Percent:
                    this._titleObject.text = Math.floor(percent * 100) + "%";
...
class GScrollBar extends GComponent {
    ...
    onGripTouchMove(evt) {
        if (!this.onStage)
            return;
        let pt = this.globalToLocal(evt.pos.x, evt.pos.y, s_vec2);
        if (this._vertical) {
    ...
    onBarTouchBegin(evt) {
        let pt = this._grip.globalToLocal(evt.pos.x, evt.pos.y, s_vec2);
        if (this._vertical) {
            if (pt.y < 0)
...
class GTree extends GList {
    ...
    getSelectedNodes(result) {
        ...
        let ret = new Array();
        for (let i = 0; i < cnt; i++) {
            let node = this.getChildAt(s_list[i])._treeNode;
            ret.push(node);
        }
        return ret;
    }
    ...
    createCell(node) {
        let child = this.getFromPool(node._resURL);
        if (!(child instanceof GComponent))
            throw new Error("cannot create tree node object.");
        child._treeNode = node;
        node._cell = child;
        let indentObj = child.getChild("indent");
        if (indentObj)
    ...
    _afterExpanded(node) {
        ...
        if (this.treeNodeRender)
            this.treeNodeRender(node, node._cell);
        let cc = node._cell.getController("expanded");
        if (cc)
            cc.selectedIndex = 1;
    ...
    _afterMoved(node) {
        let startIndex = this.getChildIndex(node._cell);
        let endIndex;
        if (node.isFolder)
    ...
    __cellMouseDown(evt) {
        let node = GObject.cast(evt.currentTarget)._treeNode;
        this._expandedStatusInEvt = node.expanded;
    }
    __expandedStateChanged(cc) {
        let node = cc.parent._treeNode;
    ...
    readItems(buffer) {
        ...
        for (i = 0; i < cnt; i++) {
            ...
            let node = new GTreeNode(isFolder, str);
            node.expanded = true;
            if (i == 0)
...
class UIObjectFactory {
    ...
    static resolveExtension(pi) {
        let extensionType = UIObjectFactory.extensions["ui://" + pi.owner.id + pi.id];
        if (!extensionType)
...
class DragDropManager {
    ...
    onDragEnd() {
        ...
        this._sourceData = null;
        let obj = GRoot.inst.touchTarget;
        while (obj) {
...
class AsyncOperation {
    createObject(pkgName, resName) {
        ...
        let pkg = UIPackage.getByName(pkgName);
        if (pkg) {
            let pi = pkg.getItemByName(resName);
            if (!pi)
...
class AsyncOperationRunner extends Component {
    ...
    init(item) {
        ...
        let di = { pi: item, type: item.objectType };
    ...
    onDestroy() {
        this._itemList.length = 0;
        let cnt = this._objectPool.length;
        if (cnt > 0) {
            for (let i = 0; i < cnt; i++)
    ...
    update() {
        ...
        let poolStart;
        let k;
        let t = game.totalTime / 1000;
        let frameTime = UIConfig.frameTimeForAsyncUIConstruction;
        let totalItems = this._itemList.length;
